...
/**
 * WebGL2 Liquid Metal Dispersion Shader Button
 * Directly ported and encapsulated from Sylva reference architecture.
 * Implements a 5-pass dispersion shader:
 *  - Prismatic Cauchy wavelength separation
 *  - Self-refraction & cursor warp displacement
 *  - Traveling chromatic rim highlight
 *  - Faceted crease ripples on click/press
 */
export default function LiquidMetalButton({
  children,
  onClick,
  className = "",
  variant = "explore", // "explore" or "play"
  ariaLabel
}) {
  ...
  useEffect(() => {
    ...
    const C = {
      glow: 1.80, glowR: 1.20, glowIn: 0.30, occl: 0.62, soften: 0.24, punch: 1.50
    };
    ...
    const handlePointerEnter = (e) => {
      if (e.pointerType === 'mouse') {
        ...
        onState.over = true; sync();
      }
    };
    const handlePointerLeave = (e) => {
      if (e.pointerType === 'mouse') {
        onState.over = false; sync();
      }
    };
    ...
    const handlePointerDown = (e) => {
      [ptr.x, ptr.y] = localPt(e);
      onState.press = true; sync();
      addRipple(ptr.x, ptr.y);
    };
    const handlePointerUp = () => { onState.press = false; sync(); };
    ...
    function renderLoop(now) {
      ...
      const idle = !onState.over && !onState.press && !onState.focus && !ripLive
  ...
  return (
    <div
      ...
      style={{ padding: '36px 48px', margin: '-36px -48px' }}
    >
      {/* Background Plate Shadow */}
      <div
        className="absolute inset-[36px_48px] rounded-full pointer-events-none transition-all duration-300"
        style={{
          background: 'linear-gradient(180deg, rgba(255,255,255,0.08), rgba(255,255,255,0.01) 44%, transparent 64%), rgba(10,12,10,0.5)',
      ...
      <button
        ...
        className="relative z-10 px-8 py-3.5 rounded-full font-mono text-xs uppercase tracking-widest text-white hover:text-accent-lime transition-colors flex items-center justify-center gap-3 cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-accent-lime"
      >
        {children}
      </button>
    </div>
